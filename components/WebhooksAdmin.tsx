    ...
    <div className="p-8">
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? "עריכת Webhook" : "Webhook חדש"}>
        <div className="space-y-4">
          ...
          <div><label className="block text-sm font-medium text-slate-700 mb-1">כרטיס (ריק = כולן)</label><input value={form.entitySlug} onChange={(e) => setForm((f) => ({ ...f, entitySlug: e.target.value }))} placeholder="leads" className="w-full rounded-lg border border-slate-300 px-3 py-2" /></div>
